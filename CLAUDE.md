...
- **AVE anomaly detection** (`core/model_ave.py`): unsupervised hybrid AE+VAE trained by `mains/ave_training.py` (EpiAtlas folds) or `mains/ave_general_training.py` (independent samples). Inference is `mains/ave_predict.py` (one model) / `mains/ave_predict_CV.py` (every fold model, plus a per-sample cross-fold summary). Those write **raw reconstruction errors only** — no threshold, no outlier flag: calling outliers on heterogeneous data needs reference distributions and cross-fold agreement, decided downstream. Scoring uses the VAE posterior mean (deterministic), not a latent sample.
