...
For AUROC metrics, see [Figure 2A,B](./sec2.html#sec-2AB)
...
def zscore_per_assay(zscore_df: pd.DataFrame) -> None:
    """
...
def merged_assays_separation_distance(zscore_df: pd.DataFrame) -> None:
...
See [Figure 2F](./sec2.html/#sec-supp-5D) section.
...
![](../resources/Supp5E.png){fig-align="center"}
...
See [Annex A](./sec2-supp.html#sec-annex-a) for a more detailled Epilogos color legend.


### F - Genome browser for biospecimen important regions

![](../resources/Supp5F.png){fig-align="center"}

Supp. Fig. 5F: Genome browser representation of the important regions shown in [Figure 2I](./sec2.html#sec-2I).
...
```{python}
#| label: score-rna-read-handling
rna_variations_dir = base_data_dir / "rna_variations"
PathChecker.check_directory(rna_variations_dir)
# The file starts with a '#'-commented provenance header (metadata source of
# true_class, model identity); comment="#" skips it.
rna_preds = pd.read_csv(
    rna_variations_dir / "rna_read_handling_predictions.csv", comment="#"
)


def score_read_handling(group: pd.DataFrame) -> pd.Series:
    """Accuracy and macro-F1 of one (task, fold, variant) group vs the true label."""
    labels = sorted(group["true_class"].unique())
    return pd.Series(
        {
...
rna_metrics = (
    rna_preds.groupby(["model", "split", "mapping"])
    .apply(score_read_handling)
    .reset_index()
)
```

```{python}
#| label: plot-rna-read-handling
#| column: screen-inset
# Canonical task order + display names; assay is excluded (mRNA/total-RNA confusion
# is not relevant to this comparison).
TASK_ORDER = ["cancer", "life_stage", "sex", "cell_type", "biomaterial"]
TASK_LABELS = {
...
MAPPINGS = ["Unique", "Unstranded (summed)", "UniqueMultiple"]
...
metrics = rna_metrics[rna_metrics["model"] != "assay"]
present = set(metrics["model"])
task_order_labels = [TASK_LABELS[t] for t in TASK_ORDER if t in present]

fig = make_subplots(
    rows=1, cols=2, subplot_titles=["Accuracy", "F1_macro"], horizontal_spacing=0.08
)
for col, metric in enumerate(["Accuracy", "F1_macro"], start=1):
    for mapping in MAPPINGS:
        sub = metrics[metrics["mapping"] == mapping]
        fig.add_trace(
            go.Box(
                x=sub["model"].map(TASK_LABELS),
                y=sub[metric],
                name=mapping,
                fillcolor=MAPPING_COLORS[mapping],
                line=dict(color="black", width=1.2),
                marker=dict(size=3, color="white", line_width=1),
                boxmean=True,
                boxpoints="all",
                pointpos=0,
                # Show the fold each point comes from on hover (as in Fig 2A,B).
                hovertemplate="%{text}",
                text=[
                    f"{split}: {value:.4f}"
                    for split, value in zip(sub["split"], sub[metric])
                ],
                legendgroup=mapping,
                showlegend=col == 1,
            ),
...
fig.update_xaxes(categoryorder="array", categoryarray=task_order_labels)
# Semi auto-scaled: fix the top, let the bottom auto-scale to the data.
fig.update_yaxes(autorange="min", range=[None, 1.02], row=1, col=1)
fig.update_yaxes(autorange="min", range=[None, 1.02], row=1, col=2)
fig.update_layout(
    boxmode="group",
...
Supp. Fig. 5G: Per-task Accuracy and macro-F1 of the RNA-Seq classifiers under three read-handling variants of the same samples — Unique (blue), Unstranded summed (green) and UniqueMultiple (orange) — each box a distribution across the 10 cross-validation folds, restricted to each fold's validation set. Including multi-mapped reads (UniqueMultiple) degrades performance, most strongly for the Sex classifier, while summing the stranded tracks (Unstranded) stays close to the trained-on Unique representation. Dashed lines represent means, solid lines the medians, boxes the quartiles.


## Supplementary Figure 6 - Prediction score threshold impact {#sec-supp6}

For full code, since the processing is more complex, see `src/python/epiclass/utils/notebooks/paper/confidence_threshold.ipynb` ([permalink](https://github.com/labjacquespe/EpiClass/blob/e87306bccecd3e35d9ffdbb5ddb29fcaa0800a35/src/python/epiclass/utils/notebooks/paper/confidence_threshold.ipynb)).

:::{.column-screen-inset}

<iframe src="../resources/all_threshold_graphs.html"
        scrolling="no"
        style="display: block; margin: 0 auto; width: 1400px; max-width: none; height: 1000px; border: none; overflow: hidden;">
</iframe>
...
For full code, since the processing is more complex, see `src/python/epiclass/utils/notebooks/paper/chromscore.ipynb` ([permalink](https://github.com/labjacquespe/EpiClass/blob/94e0f352ec631eeab203f28e885ecd9fc72b2acc/src/python/epiclass/utils/notebooks/paper/chromscore.ipynb)).

This is the breakdown per cell type of [Figure 2G](./sec2.html#sec-2G).

<iframe src="../resources/16ct_chromscore.html"
        scrolling="no"
        style="display: block; margin: 0 auto; width: 100%; max-width: 1010px; aspect-ratio: 1010 / 910; border: 0; overflow: hidden;">
</iframe>
...
![](../resources/epilogos_legend.png){fig-align="center"}
