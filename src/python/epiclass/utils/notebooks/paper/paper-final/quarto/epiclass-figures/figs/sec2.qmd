...
def plot_shannon_entropy(class_balance_df: pd.DataFrame, ordered_task_names: List[str]|None) -> None:
    """Graph Shannon entropy values, in the order given by ordered_task_names."""
    df = class_balance_df.copy()

    if ordered_task_names:
        df = df.loc[[t for t in ordered_task_names if t in df.index]]
...
1. For each bigwig file, the chrY average value is computed with the pyBigWig module, in `chrY_bigwig_mean.py` ([permalink](https://github.com/labjacquespe/EpiClass/blob/e87306bccecd3e35d9ffdbb5ddb29fcaa0800a35/src/python/epiclass/utils/chrY_bigwig_mean.py)).
...
def graph_gp_age(
    ...
    Args:
        ...
        title: Optional suffix appended to the plot title.
    """
...
Fig 2F: Distribution of the age prediction from GP-age for the WGBS datasets with an originally unknown life stage predicted by EpiClass (datasets related to blood biospecimen are in red, others in grey). Boxplot elements are as in [Fig. 2A](./sec2.html#sec-2AB).
...
For full code, since the processing is more complex, see `src/python/epiclass/utils/notebooks/analyze_hdf5_vals.ipynb` ([permalink](https://github.com/labjacquespe/EpiClass/blob/e87306bccecd3e35d9ffdbb5ddb29fcaa0800a35/src/python/epiclass/utils/notebooks/analyze_hdf5_vals.ipynb)), particularly section "ChromScore hdf5 values".

<iframe src="../resources/global_chromscore.html"
        scrolling="no"
        style="display: block; margin: 0 auto; width: 100%; max-width: 510px; aspect-ratio: 1 / 1; border: 0; overflow: hidden;">
</iframe>

Fig 2G: Distribution of the average ChromScore values over the important Biospecimen classifier regions according to SHAP (blue) compared to the global distribution (pink). Statistical significance was assessed using a two-sided Welch's t-test. Boxplot elements are as in [Fig. 2A](./sec2.html#sec-2AB), with a violin representation on top.


### H - Gene Ontology and SHAP values {#sec-2H}

See `src/python/epiclass/utils/notebooks/profile_bed.ipynb` ([permalink](https://github.com/labjacquespe/EpiClass/blob/e87306bccecd3e35d9ffdbb5ddb29fcaa0800a35/src/python/epiclass/utils/notebooks/profile_bed.ipynb)) for creation of gene ontology files.
...
def plot_go_heatmap(table: pd.DataFrame, height: int, title: str|None=None):
    """Plot a GO heatmap."""
    # Define colorbar
    sigma = "\u03c3"
...
![](../resources/fig2I.png){fig-align="center"}

Fig. 2I: Epilogos visualization of one of the important Biospecimen classifier regions enriched in the `T cell receptor complex` GO term from [Fig. 2H](./sec2.html#sec-2H).

See [Annex A](./sec2.html#sec-annex-a) for a more detailled Epilogos color legend.
...
def plot_cnv_zscores(cnv_df: pd.DataFrame) -> None:
...
def plot_cnv_zscores_alt(cnv_df: pd.DataFrame) -> None:
...
See the [Additional Supplementary Figures page](./sec2-supp.html) (separate to limit maximum website page size and loading times).
